...
export default function CampusConnectApp() {
  ...
  const [registeredAccounts, setRegisteredAccounts] = useState(() => {
    try { return JSON.parse(localStorage.getItem("cc_accounts") || "[]"); } catch { return []; }
  });
  const saveAccounts = (accs) => { setRegisteredAccounts(accs); try { localStorage.setItem("cc_accounts", JSON.stringify(accs)); } catch {} };
  ...
  const [profile, setProfile] = useState(() => { try { const p = localStorage.getItem("cc_photo"); return { name: "", email: "", phone: "", faculty: "", studentId: "", photo: p || "" }; } catch { return { name: "", email: "", phone: "", faculty: "", studentId: "", photo: "" }; } });
  ...
  const handlePhoto = (e) => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = (ev) => { const img = ev.target.result; setProfile(p => ({ ...p, photo: img })); try { localStorage.setItem("cc_photo", img); } catch {} }; reader.readAsDataURL(file); };
  ...
  const verifyOtp = () => {
    ...
    if (enteredOtp.trim() === generatedOtp) {
      ...
      // Save account if new signup
      if (!registeredAccounts.some(a => a.email === pendingAuth.email.toLowerCase())) {
        const pw = signupPass || loginPass;
        saveAccounts([...registeredAccounts, { email: pendingAuth.email.toLowerCase(), name: pendingAuth.name, password: pw }]);
      }
      setIsLoggedIn(true);
      setOtpStep("form");
      setEnteredOtp("");
  ...
  // === LOGIN ===
  if (!isLoggedIn) {
    const pwCheck = checkPassword(signupPass);
    const handleLogin = async () => {
      setLoginError("");
      if (!loginEmail.includes("@")) { setLoginError("Enter a valid email address."); return; }
      if (loginPass.length < 1) { setLoginError("Enter your password."); return; }
      const acc = registeredAccounts.find(a => a.email === loginEmail.toLowerCase());
      if (!acc) { setLoginError("No account found with this email. Please sign up first."); return; }
      if (acc.password !== loginPass) { setLoginError("Incorrect password."); return; }
      await sendOtp(loginEmail, acc.name);
    };
    const handleSignup = async () => {
      ...
      if (registeredAccounts.some(a => a.email === signupEmail.toLowerCase())) { setLoginError("An account with this email already exists. Please log in instead."); return; }
  ...
  const SideMenu = () => (<><div onClick={() => setMenuOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 140 }} /><div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: 290, maxWidth: "78vw", background: C.primary, zIndex: 150, overflowY: "auto", boxShadow: "-4px 0 24px rgba(0,0,0,0.3)", paddingTop: "env(safe-area-inset-top, 0px)" }}><button onClick={() => setMenuOpen(false)} style={{ position: "absolute", top: "max(14px, env(safe-area-inset-top, 14px))", right: 14, background: "none", border: "none", color: "#fff", cursor: "pointer", zIndex: 10 }}><X size={22} /></button><div style={{ background: "rgba(255,255,255,0.08)", padding: "28px 20px 20px", borderBottom: "1px solid rgba(255,255,255,0.1)" }}><div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}><div style={{ width: 48, height: 48, borderRadius: "50%", background: "rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>{profile.photo ? <img src={profile.photo} style={{ width: 48, height: 48, objectFit: "cover" }} alt="" /> : <User size={24} color="#fff" />}</div><div><div style={{ fontSize: 16, fontWeight: 700, color: "#fff" }}>{profile.name || "Set up profile"}</div><div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)" }}>{profile.email}</div></div></div><button onClick={() => nav("profile")} style={{ width: "100%", padding: "8px 14px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.25)", background: "rgba(255,255,255,0.08)", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}><User size={14} style={{ marginRight: 6, verticalAlign: "middle" }} />Edit Profile</button></div><div style={{ padding: "12px 16px" }}>{[{ l: "Home", i: <Home size={18} />, s: "home" }, { l: "Study Groups", i: <BookOpen size={18} />, s: "groups" }, { l: "Events", i: <Calendar size={18} />, s: "events" }, { l: "Peer Support", i: <Heart size={18} />, s: "support" }, { l: "Campus Connect+", i: <Star size={18} />, s: "subscribe" }, { l: "Contact Us", i: <Phone size={18} />, s: "contact" }, { l: "About & Services", i: <Info size={18} />, s: "about" }].map(x => (<button key={x.s} onClick={() => nav(x.s)} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "13px 10px", background: screen === x.s ? "rgba(255,255,255,0.12)" : "none", border: "none", color: "#fff", fontSize: 15, fontWeight: screen === x.s ? 700 : 400, cursor: "pointer", borderRadius: 8, textAlign: "left", fontFamily: "inherit" }}>{x.i}{x.l}</button>))}</div><div style={{ padding: "12px 16px", borderTop: "1px solid rgba(255,255,255,0.1)" }}><button onClick={() => { setIsLoggedIn(false); setMenuOpen(false); setScreen("home"); setScreenStack([]); setLoginEmail(""); setLoginPass(""); setSignupName(""); setSignupEmail(""); setSignupPass(""); setShowPass(false); setLoginError(""); setOtpStep("form"); setEnteredOtp(""); }} style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "13px 10px", background: "none", border: "none", color: "#ff6b6b", fontSize: 15, fontWeight: 600, cursor: "pointer", borderRadius: 8, fontFamily: "inherit" }}><LogOut size={18} />Log Out</button></div></div></>);

  const ProfileScreen = () => (<><Header title="My Profile" back /><div style={{ padding: "16px 20px", paddingBottom: 90 }}><div style={{ textAlign: "center", marginBottom: 24 }}><label style={{ cursor: "pointer", display: "block", width: 80, height: 80, margin: "0 auto 10px", position: "relative" }}><div style={{ width: 80, height: 80, borderRadius: "50%", background: C.accent + "20", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>{profile.photo ? <img src={profile.photo} style={{ width: 80, height: 80, objectFit: "cover" }} alt="" /> : <User size={36} color={C.accent} />}</div><div style={{ position: "absolute", bottom: 0, right: 0, width: 24, height: 24, borderRadius: "50%", background: C.accent, display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid #fff" }}><Plus size={12} color="#fff" /></div><input type="file" accept="image/*" onChange={handlePhoto} style={{ display: "none" }} /></label><div style={{ fontSize: 18, fontWeight: 700, color: C.text }}>{profile.name || "Student"}</div><div style={{ fontSize: 13, color: C.textSec }}>{profile.faculty || "No faculty selected"}</div></div><div style={scard}><div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 16 }}>Personal Details</div><label style={{ fontSize: 13, fontWeight: 600, color: C.textSec, display: "block", marginBottom: 4 }}>Full Name</label><input value={profile.name} onChange={e => setProfile({ ...profile, name: e.target.value })} placeholder="Enter your full name" style={{ ...si, marginBottom: 14 }} /><label style={{ fontSize: 13, fontWeight: 600, color: C.textSec, display: "block", marginBottom: 4 }}>Email</label><input value={profile.email} onChange={e => setProfile({ ...profile, email: e.target.value })} placeholder="you@student.vu.edu.au" style={{ ...si, marginBottom: 14 }} /><label style={{ fontSize: 13, fontWeight: 600, color: C.textSec, display: "block", marginBottom: 4 }}>Phone Number</label><input value={profile.phone} onChange={e => setProfile({ ...profile, phone: e.target.value })} placeholder="+61 4XX XXX XXX" style={{ ...si, marginBottom: 14 }} /><label style={{ fontSize: 13, fontWeight: 600, color: C.textSec, display: "block", marginBottom: 4 }}>Student ID</label><input value={profile.studentId} onChange={e => setProfile({ ...profile, studentId: e.target.value })} placeholder="e.g. s8112534" style={{ ...si, marginBottom: 14 }} /><label style={{ fontSize: 13, fontWeight: 600, color: C.textSec, display: "block", marginBottom: 4 }}>Faculty</label><select value={profile.faculty} onChange={e => setProfile({ ...profile, faculty: e.target.value })} style={{ ...si, marginBottom: 20, appearance: "auto" }}><option value="">Select your faculty</option>{FACULTIES.map(f => <option key={f} value={f}>{f}</option>)}</select><button onClick={() => { setProfileSaved(true); showToast("Profile saved!"); setTimeout(() => setProfileSaved(false), 2000); }} style={{ ...sbtn, background: profileSaved ? C.success : C.accent }}>{profileSaved ? <><Check size={16} /> Saved</> : "Save Profile"}</button></div>
    {/* Delete Account */}
    {!showDeleteConfirm ? (
      <button onClick={() => setShowDeleteConfirm(true)} style={{ ...sbtn, background: "transparent", color: C.danger, border: `2px solid ${C.danger}`, marginTop: 8 }}><AlertCircle size={16} /> Delete Account</button>
    ) : (
      <div style={{ ...scard, borderColor: C.danger + "40", background: "#fff5f5", marginTop: 8 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: C.danger, marginBottom: 6 }}>Delete your account?</div>
        <p style={{ fontSize: 13, color: C.textSec, marginBottom: 14 }}>This will permanently remove your account and all data. You can sign up again with the same email after deletion.</p>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => { const updated = registeredAccounts.filter(a => a.email !== profile.email.toLowerCase()); saveAccounts(updated); setIsLoggedIn(false); setScreen("home"); setScreenStack([]); setShowDeleteConfirm(false); setProfile({ name: "", email: "", phone: "", faculty: "", studentId: "" }); showToast("Account deleted."); }} style={{ ...sbtn, background: C.danger, flex: 1 }}>Yes, Delete</button>
          <button onClick={() => setShowDeleteConfirm(false)} style={{ ...sbtn, background: "transparent", color: C.textSec, border: `1.5px solid ${C.border}`, flex: 1 }}>Cancel</button>
        </div>
      </div>
    )}
    </div></>);
  ...
  const ContactScreen = () => (<><Header title="Contact Us" back /><div style={{ padding: "16px 20px", paddingBottom: 90 }}><div style={scard}><div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 14 }}>Get in Touch</div><input placeholder="Your Name" style={{ ...si, marginBottom: 10 }} /><input placeholder="Your Email" style={{ ...si, marginBottom: 10 }} /><textarea placeholder="Your Message" rows={4} style={{ ...si, resize: "none", marginBottom: 10 }} /><button onClick={() => showToast("Message sent!")} style={sbtn}><Send size={16} /> Send Message</button></div><div style={scard}><div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 12 }}>Contact Details</div>{[{ i: <Mail size={16} />, t: "hello@campusconnect.com.au" }, { i: <Phone size={16} />, t: "+61 3 9919 6100" }, { i: <MapPin size={16} />, t: "Victoria University, Footscray Park, Ballarat Rd, VIC 3011" }].map((c, j) => (<div key={j} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 0", fontSize: 14, color: C.text }}><span style={{ color: C.accent, marginTop: 2 }}>{c.i}</span><span>{c.t}</span></div>))}</div><div style={scard}><div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 12 }}>Follow Us</div><div style={{ display: "flex", gap: 10 }}>{[{ n: "Instagram", c: "#E1306C", i: <Instagram size={20} />, url: "https://www.instagram.com/campusconnectvu/" }, { n: "TikTok", c: "#000", i: <ExternalLink size={20} />, url: "https://www.tiktok.com/@campusconnectvu" }, { n: "LinkedIn", c: "#0077B5", i: <ExternalLink size={20} />, url: "https://www.linkedin.com/company/campusconnectvu" }].map(s => (<a key={s.n} href={s.url} target="_blank" rel="noopener noreferrer" style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, padding: "12px 8px", borderRadius: 12, border: `1px solid ${C.border}`, background: "#fff", cursor: "pointer", fontFamily: "inherit", textDecoration: "none" }}><span style={{ color: s.c }}>{s.i}</span><span style={{ fontSize: 11, fontWeight: 600, color: C.text }}>{s.n}</span></a>))}</div><a href="https://campus-connect-henna-nine.vercel.app" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 12, padding: "10px", borderRadius: 10, background: C.accent, color: "#fff", textDecoration: "none", fontSize: 13, fontWeight: 600 }}><ExternalLink size={14} /> Install Campus Connect App</a></div><div style={scard}><div style={{ fontSize: 16, fontWeight: 700, color: C.text, marginBottom: 12 }}>Our Location</div><div style={{ borderRadius: 12, overflow: "hidden", border: `1px solid ${C.border}` }}><iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3152.8!2d144.8976!3d-37.7983!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x6ad65d1cc5a935b1%3A0xc04f2e7e82060e25!2sVictoria%20University%20Footscray%20Park%20Campus!5e0!3m2!1sen!2sau!4v1" width="100%" height="200" style={{ border: 0 }} allowFullScreen="" loading="lazy" title="VU Map" /></div></div></div></>);
